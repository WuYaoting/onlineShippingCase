...
<script type="text/javascript">
		...
		function fqBrand(bid) {
			// 其实不用判断price是否为null,因为他不可能为null,如果从controller传过来的price为null,则'${price}'为''
			if(price != null && price != "") {
				alert(bid);
				window.location.href = "/search?keyword="+keyword+"&price="+price+"&brandId="+bid;
			} else {
		...
		function fqPrice(pri) {
			if(brandId != null && brandId != "") {
				window.location.href = "/search?keyword="+keyword+"&price="+pri+"&brandId"+brandId;
			} else {
		...
		// 删除筛选条件
		function delCondition(value) {
			if(price != "" && brandId != "") {
				if(value == price || value == price + "以上") {
			...
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="依琦莲（yiqilian）"  href="javascript:;" onclick="delCondition('${m.value}')" class="crumb-select-item">
